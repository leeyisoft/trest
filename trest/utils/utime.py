...
if __name__ == '__main__':
    import sys
    import string
    t1 = '2018-02-27 02:13:02.087558+08:00'
    ts1 = 1519668782.087558
    print(ts_to_str(1519668782.087558))
    print(ts_to_str(1519668782.087558, to_tz='Etc/GMT+9'))
    print(ts1)
    print(str_to_datetime(t1))
    # print(t1)
    print(str_to_datetime(t1, 'Etc/GMT+9'))
    # print('dd ', str_to_datetime('2018-02-27 02:13:02.087558'))
    # sys.exit(0)
    """ 测试脚本
    python applications/core/utils/utime.py
    """
    dt, ts, local_now = dt_now(), timestamp(), datetime.datetime.now()
    print('ts ', ts, 'dt ', dt)
    local_dt = dt_to_timezone(dt, 'PRC')
    # assert local_dt == local_now
    print(local_dt, 'dt_to_timezone PRC ')
    print(local_now, 'local_now       PRC ')
    dt2 = ts_to_datetime(ts)
    print(dt, 'dt1 UTC ')
    print(dt2, 'dt2 UTC')
    # assert str(dt)==str(dt2)
    ts2 = datetime_to_ts(dt2)
    print(ts, 'ts')
    print(ts2, 'ts2')
    # assert ts==ts2
    ts1,ts2,ts3 = time.time(),datetime.datetime.now().timestamp(),datetime.datetime.utcnow().timestamp(),
    print('timestamp ')
    print(ts1)
    print(ts2)
    print(ts3)
